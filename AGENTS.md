...
"$gh_bin" attestation verify zux \
  ...
  --cert-identity "https://github.com/hrzlgnm/zux/.github/workflows/release.yml@refs/heads/main"
```
...
- This is a **binary** attestation (over `zux_linux_x64`). Do **not** pass
  `--source-digest` here.
- `cert-identity` pin:
  `https://github.com/hrzlgnm/zux/.github/workflows/release.yml@refs/heads/main`
  (upstream releases via release-please, so the workflow runs on `main` —
  the tag is only created alongside, hence `@refs/heads/main`)
